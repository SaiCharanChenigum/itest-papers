...
export default function ChapterViewerClient({ contents, hasPremiumAccess }: ChapterViewerClientProps) {
    ...
    useEffect(() => {
        if (activeItem && canAccessItem) {
            fetch("/api/progress", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ contentId: activeItem.id, completed: false })
            }).catch(err => console.error("Error recording access:", err));
        }
    }, [activeItem?.id, canAccessItem]);

    const handlePageChange = (e: { currentPage: number, doc: any }) => {
        const totalPages = e.doc.numPages;
        // Mark as completed if reached the last page (0-indexed)
        if (e.currentPage === totalPages - 1 && !completedItems.has(activeItem.id)) {
            fetch("/api/progress", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ contentId: activeItem.id, completed: true })
            }).then(res => {
                if (res.ok) {
                    setCompletedItems(prev => new Set(prev).add(activeItem.id));
                }
            ...
            <div className="flex flex-wrap gap-2 border-b border-border pb-4">
                {contents.map((item, index) => (
                    <button
                        ...
                        className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors border flex items-center gap-2 ${activeIndex === index
                            ? "bg-primary text-primary-foreground border-primary shadow-sm"
                            : "bg-card text-foreground/80 hover:bg-accent hover:text-foreground border-border"
                            }`}
                    >
                        {item.type.replace('_', ' ')}
                        {completedItems.has(item.id) && <CheckCircle2 className="w-3.5 h-3.5 text-secondary-foreground" />}
